...
If[$SYMMETRIZE === 1,
	...
	import = Flatten[Import["$jobdir/dists/BS"<>ToString[currstrap]<>"/"<>ToString[curriter-1]<>"/"<>"$MATRIX"<>"-t-"<>"$CONFIGS"<>"-averaged.txt","Data"]]
	];
];
...
If[$MATRIX===1,       
	For[i=0,i<loopsize,i++,
		...
		rgstep = Table[With[{
                	t1=If[RandomReal[]>pc,generatedfromdist[[RandomInteger[{1, configsize}]]],1],
                	t2=If[RandomReal[]>pc,generatedfromdist[[RandomInteger[{1, configsize}]]],1],
                	t3=If[RandomReal[]>pc,generatedfromdist[[RandomInteger[{1, configsize}]]],1],
                	t4=If[RandomReal[]>pc,generatedfromdist[[RandomInteger[{1, configsize}]]],1],
                	t5=If[RandomReal[]>pc,generatedfromdist[[RandomInteger[{1, configsize}]]],1]},
...
Print["---Exporting symmetrised distribution to: "<>"$jobdir/dists/BS"<>ToString[currstrap]<>"/"<>ToString[curriter]<>"/"<>"$MATRIX"<>"-z-"<>"$CONFIGS"<>"-averaged-symmetrized.txt"];
